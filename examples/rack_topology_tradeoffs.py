...
def analyze_topology_pair(cfg: RackConfig) -> None:
    ...
    print(f"  {cfg.name}")
    print(
        f"  {cfg.gpu_count} GPUs × {cfg.tdp_per_gpu_w:.0f} W = "
    ...
    row("Total CDU flow (LPM)", f"{cfg.flow_per_gpu_lpm:.1f}", f"{cfg.flow_per_gpu_lpm * cfg.gpu_count:.1f}", "← series needs N× less flow")

    row("Total ΔP (kPa)", f"{series.total_pressure_drop_pa / 1000:.1f}", f"{parallel.total_pressure_drop_pa / 1000:.1f}", "← series accumulates N× ΔP")

    row("Total pump power (W)", f"{series.total_pump_power_w:.1f}", f"{parallel.total_pump_power_w:.1f}")

    row("CDU return temp (°C)", f"{series.cdu_outlet_temp_c:.1f}", f"{parallel.cdu_outlet_temp_c:.1f}")

    row("Max junction temp (°C)", f"{series.max_junction_temp_c:.1f}", f"{parallel.max_junction_temp_c:.1f}", "← parallel wins (no Tj stacking)")

    margin_s = cfg.tj_limit_c - series.max_junction_temp_c
    margin_p = cfg.tj_limit_c - parallel.max_junction_temp_c
    row(f"Tj margin vs {cfg.tj_limit_c:.0f}°C limit", f"{margin_s:+.1f}°C", f"{margin_p:+.1f}°C")

    row("Hottest GPU index (0-based)", f"{series.hottest_gpu_index}", f"{parallel.hottest_gpu_index}")
...
def section_coolant_rise_explanation() -> None:
    ...
    print(f"  Total CDU temperature rise: {rack.cdu_outlet_temp_c - 25.0:.2f}°C ({25.0}°C supply → {rack.cdu_outlet_temp_c:.2f}°C return)")
    print()
